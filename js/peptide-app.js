...
const residueChemistry = {
  A: { propertyTags: ["Nonpolar", "Aliphatic"], pka: { cTerm: 2.34, nTerm: 9.69 }, sideChainPka: null, sideChainChargeType: null },
  C: { propertyTags: ["Polar", "Uncharged"], pka: { cTerm: 1.96, nTerm: 10.28 }, sideChainPka: 8.18, sideChainChargeType: "acidic" },
  D: { propertyTags: ["Negatively Charged", "Acidic"], pka: { cTerm: 1.88, nTerm: 9.60 }, sideChainPka: 3.65, sideChainChargeType: "acidic" },
  E: { propertyTags: ["Negatively Charged", "Acidic"], pka: { cTerm: 2.19, nTerm: 9.67 }, sideChainPka: 4.25, sideChainChargeType: "acidic" },
  F: { propertyTags: ["Aromatic", "Nonpolar"], pka: { cTerm: 1.83, nTerm: 9.13 }, sideChainPka: null, sideChainChargeType: null },
  G: { propertyTags: ["Nonpolar", "Aliphatic"], pka: { cTerm: 2.34, nTerm: 9.60 }, sideChainPka: null, sideChainChargeType: null },
  H: { propertyTags: ["Positively Charged", "Basic"], pka: { cTerm: 1.82, nTerm: 9.17 }, sideChainPka: 6.00, sideChainChargeType: "basic" },
  I: { propertyTags: ["Nonpolar", "Aliphatic"], pka: { cTerm: 2.36, nTerm: 9.68 }, sideChainPka: null, sideChainChargeType: null },
  K: { propertyTags: ["Positively Charged", "Basic"], pka: { cTerm: 2.18, nTerm: 8.95 }, sideChainPka: 10.53, sideChainChargeType: "basic" },
  L: { propertyTags: ["Nonpolar", "Aliphatic"], pka: { cTerm: 2.36, nTerm: 9.60 }, sideChainPka: null, sideChainChargeType: null },
  M: { propertyTags: ["Nonpolar", "Aliphatic"], pka: { cTerm: 2.28, nTerm: 9.21 }, sideChainPka: null, sideChainChargeType: null },
  N: { propertyTags: ["Polar", "Uncharged"], pka: { cTerm: 2.02, nTerm: 8.80 }, sideChainPka: null, sideChainChargeType: null },
  P: { propertyTags: ["Nonpolar", "Aliphatic"], pka: { cTerm: 1.99, nTerm: 10.96 }, sideChainPka: null, sideChainChargeType: null },
  Q: { propertyTags: ["Polar", "Uncharged"], pka: { cTerm: 2.17, nTerm: 9.13 }, sideChainPka: null, sideChainChargeType: null },
  R: { propertyTags: ["Positively Charged", "Basic"], pka: { cTerm: 2.17, nTerm: 9.04 }, sideChainPka: 12.48, sideChainChargeType: "basic" },
  S: { propertyTags: ["Polar", "Uncharged"], pka: { cTerm: 2.21, nTerm: 9.15 }, sideChainPka: null, sideChainChargeType: null },
  T: { propertyTags: ["Polar", "Uncharged"], pka: { cTerm: 2.11, nTerm: 9.62 }, sideChainPka: null, sideChainChargeType: null },
  V: { propertyTags: ["Nonpolar", "Aliphatic"], pka: { cTerm: 2.32, nTerm: 9.62 }, sideChainPka: null, sideChainChargeType: null },
  W: { propertyTags: ["Aromatic", "Nonpolar"], pka: { cTerm: 2.38, nTerm: 9.39 }, sideChainPka: null, sideChainChargeType: null },
  Y: { propertyTags: ["Aromatic", "Polar"], pka: { cTerm: 2.20, nTerm: 9.11 }, sideChainPka: 10.07, sideChainChargeType: "acidic" },
};
...
const dom = {
  ...
  phDisplay: document.querySelector("#ph-display"),
...
function buildPeptideSmiles(sequence) {
  ...
  for (const code of sequence.split("")) {
    const fragment = peptideSmilesFragments[code];
    if (fragment.includes("1") || fragment.includes("2")) {
      const ring1 = currentRing < 10 ? currentRing.toString() : `%${currentRing}`;
      const ring2 = (currentRing + 1) < 10 ? (currentRing + 1).toString() : `%${currentRing + 1}`;
      
      let mapped = "";
      for (let i = 0; i < fragment.length; i++) {
        if (fragment[i] === "1") mapped += ring1;
        else if (fragment[i] === "2") mapped += ring2;
        else mapped += fragment[i];
      }
      smiles += mapped;
      
      currentRing += fragment.includes("2") ? 2 : 1;
    } else {
      smiles += fragment;
    }
  }
...
function updateSummary(mappedPeptide) {
  ...
  dom.phDisplay.textContent = pH.toFixed(2);
...
async function renderViewer(mappedPeptide) {
  ...
  try {
    const molText = await buildStructureMolfile(mappedPeptide.sequence);
    if (requestId !== currentViewerRequest) {
      return;
    }

    latestModelMol = molText;
    ...
    const loadedFormat = await loadMolfileIntoViewer(activeViewer, molText);
...
function triggerDownload() {
  ...
  const sequence = dom.sequence.textContent === "-" ? "peptide" : dom.sequence.textContent;
  const blob = new Blob([latestModelMol], { type: "chemical/x-mdl-molfile" });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
